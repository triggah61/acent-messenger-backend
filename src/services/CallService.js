...
class CallService {
   ...
   */
  async initiateCall({ initiatorId, participantIds, type, chatSessionId = null, metadata = {} }) {
    try {
      console.log(`CallService: Initiating ${type} call from ${initiatorId} to ${participantIds.join(', ')}`);
      
      // Validate input parameters
      if (!initiatorId || !participantIds || !Array.isArray(participantIds) || participantIds.length === 0) {
        throw new Error('Invalid call parameters: initiatorId and participantIds are required');
      }
      ...
      if (activeCallsForParticipants.length > 0) {
        return {
          ...
          busyParticipants: activeCallsForParticipants.map(call => call.participants.map(p => p.user))
        };
      }
      ...
      await call.populate('initiator', 'firstName lastName photo dialCode phone');
      await call.populate('participants.user', 'firstName lastName photo dialCode phone');
      
      console.log(`CallService: Call created with ID ${call._id} and channel ${channelName}`);
  ...
  async acceptCall(callId, userId) {
    try {
      ...
      // If all participants have responded, start the call
      const allResponded = call.participants.every(p => 
        p.status === 'accepted' || p.status === 'declined'
      );
      
      if (allResponded && call.participants.some(p => p.status === 'accepted')) {
        call.status = 'active';
  ...
  async declineCall(callId, userId) {
    try {
      ...
      const allDeclined = call.participants.filter(p => p.role === 'callee')
  ...
  async endCall(callId, userId, reason = 'normal') {
    try {
      console.log(`CallService: User ${userId} ending call ${callId} with reason: ${reason}`);
  ...
  async generateCallToken(callId, userId) {
    try {
      console.log(`CallService: Generating token for user ${userId} in call ${callId}`);
      ...
      console.log(`CallService: Token generated successfully for user ${userId} in call ${callId}`);
  ...
  async getCallHistory(userId, options = {}) {
    try {
      ...
      console.log(`CallService: Retrieved ${result.docs.length} call history records for user ${userId}`);
  ...
  async sendCallInvitation(call) {
    try {
      ...
      console.log(`CallService: Call invitation sent to ${call.participants.length - 1} participants`);
  ...
  async sendCallUpdate(call, event, data = {}) {
    try {
      ...
      console.log(`CallService: Call update "${event}" sent to ${call.participants.length} participants`);
      ...
      for (const participant of call.participants) {
        if (participant.role === 'callee') {
          ...
          FCMService.sendIncomingCallNotification(
            ...
            console.error(`CallService: Error sending FCM notification to ${participant.user._id}:`, error);
          });
        }
      }
