...
router.post("/auth", async (req, res) => {
  try {
    ...
    console.log(`Pusher auth request: User ${userId}, Channel: ${channel_name}, Socket: ${socket_id}`);
    ...
    if (channel_name.startsWith('private-user_')) {
      ...
      const fullName = `${firstName} ${lastName}`.trim() || req.user.username || 'User';
      ...
      const authData = global.pusher.authenticate(socket_id, channel_name, presenceData);
    ...
    switch (event) {
      ...
      case 'mark_message_read':
        if (data.messageId && data.chatSessionId) {
          await sendMessageReadStatus(data.chatSessionId, data.messageId, userId);
          res.json({ success: true, message: 'Message read status sent' });
        } else {
          res.status(400).json({ error: 'Message ID and chat session ID required' });
        }
        break;
      ...
      case 'message_reaction':
        if (data.messageId && data.chatSessionId && data.reaction) {
          ...
          res.status(400).json({ error: 'Message ID, chat session ID, and reaction required' });
        }
        break;
